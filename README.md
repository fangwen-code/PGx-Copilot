...
- **三层确定性架构**：CPIC 精确匹配 + 规则引擎 + RAG 背景，结论走确定性、背景靠检索。
- **检索质量层**：自研 Query Expansion → Dense+BM25 混合 → Cross-encoder 重排序 → 证据过滤，并建立可复现评测（`eval.py`）作为回归基准。
- **CPIC 多键回退匹配**：等位基因型 / 活性评分 / 表型三种存储键自动回退（如 CYP2D6-metoprolol `*4/*4` → 活性评分 0 → Poor Metabolizer）；支持 CYP2C9+VKORC1 → 华法林多基因联合查询。
- **多层级防幻觉**：确定性推荐、Writer-Reviewer 证据核验、证据不足拒答、每节来源标注（CPIC Guideline / PubMed PMID / 中文指南文献标题）、检索证据可视化可核验。
- **质量闭环**：Coverage Gap 主动监控（`coverage_gaps.jsonl`）驱动知识库迭代。
